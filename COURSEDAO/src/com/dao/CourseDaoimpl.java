...
public class CourseDaoimpl implements courseDao
{
	...
	public Course getCourse(int cid) {
		...
		try {
			...
			int duration=rs.getInt(COLcduration);
			int fees =rs.getInt(COLfees);
			
			Course  course =new Course(cid,duration,name,fees);
	...
	public void updateCourse(Course c) {
		...
		{
		Connection connection=DBConnection.getConnection();
		String sqlQuery="update "+TABLEcourse+" set "+COLcname+" = ? ,"+COLcduration+" = ?,"+COLfees+" = ? where "+COLcid+" ="+cid1;
		
		PreparedStatement pst = connection.prepareStatement(sqlQuery);
		pst.setString(1, c.getCname());
		pst.setInt(2, c.getCduration());
		pst.setInt(3, c.getFees());
